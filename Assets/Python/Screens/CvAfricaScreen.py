...
class CvAfricaScreen:
	...
	def drawContents(self):
		...
		for iUnit in self.AfricaUnitsList:
			...
			elif (iCargoCount == 0) and (gc.getUnitInfo(unit.getUnitType()).getAfricaCost() > 0) and (iSeaUnitCount > 1) and (self.iSellShip > 0):
	...
	def getShipSellPrice(self, iUnit):
		if iUnit != -1:
			...
			if player.getUnit(iUnit).getUnitType() > -1:
				iSellPrice = self.iSellShip * gc.getUnitInfo(player.getUnit(iUnit).getUnitType()).getAfricaCost() * iTrainPercent / 1200
