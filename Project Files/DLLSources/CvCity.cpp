	...
	if(!isNative())
	{
		for (YieldTypes eYield = FIRST_YIELD; eYield < NUM_YIELD_TYPES; ++eYield)
		{
			...
			if (kYield.isCargo())
			{
				...
				// produced
				else if (eYield == YIELD_RICE || eYield == YIELD_CASSAVA || eYield == YIELD_COCA_LEAVES || eYield == YIELD_MAPLE_SIRUP || eYield == YIELD_WILD_FEATHERS || eYield == YIELD_SILVER || eYield == YIELD_GOLD || eYield == YIELD_GEMS || eYield == YIELD_COCOA || eYield == YIELD_COFFEE || eYield == YIELD_ROASTED_PEANUTS || eYield == YIELD_SALT || eYield == YIELD_SPICES || eYield == YIELD_VANILLA || eYield == YIELD_CHOCOLATE || eYield == YIELD_CLOTH || eYield == YIELD_COLOURED_CLOTH || eYield == YIELD_WOOL_CLOTH || eYield == YIELD_COLOURED_WOOL_CLOTH || eYield == YIELD_PIG_LEATHER || eYield == YIELD_LEATHER || eYield == YIELD_GOAT_HIDE_BOOTS || eYield == YIELD_PADDED_LEATHER_COATS || eYield == YIELD_COATS || eYield == YIELD_PREMIUM_COATS || eYield == YIELD_BEER || eYield == YIELD_RUM || eYield == YIELD_HOOCH || eYield == YIELD_WINE || eYield == YIELD_OLIVE_OIL || eYield == YIELD_RAPE_OIL || eYield == YIELD_CIGARS || eYield == YIELD_YERBA_TEA || eYield == YIELD_WHALE_OIL || eYield == YIELD_FURNITURE || eYield == YIELD_PADDED_FURNITURE)
				{
