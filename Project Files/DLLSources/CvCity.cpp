...
void CvCity::ejectMissionariesAndTraders()
{
	int missionaryRate = getMissionaryRate();
	if (missionaryRate > 0) {
		PlayerTypes missionaryPlayer = getMissionaryPlayer();

		UnitTypes EjectedMissionaryType = (UnitTypes)GC.getCivilizationInfo(getCivilizationType()).getCivilizationUnits(GC.getDefineINT("UNITCLASS_COLONIST"));

		if (missionaryRate > GC.getProfessionInfo(PROFESSION_MISSIONARY).getMissionaryRate() ) {
			EjectedMissionaryType = (UnitTypes)GC.getCivilizationInfo(getCivilizationType()).getCivilizationUnits(GC.getDefineINT("UNITCLASS_CHRISTIAN_MISSIONARY"));
		}
	...
	if (nativeTradeRate > 0) {
		...
		UnitTypes EjectedTraderType = (UnitTypes)GC.getCivilizationInfo(getCivilizationType()).getCivilizationUnits(GC.getDefineINT("UNITCLASS_COLONIST"));

		if (nativeTradeRate > GC.getProfessionInfo(PROFESSION_NATIVE_TRADER).getNativeTradeRate() ) {
			EjectedTraderType = (UnitTypes)GC.getCivilizationInfo(getCivilizationType()).getCivilizationUnits(GC.getDefineINT("UNITCLASS_EXPERT_TRADER"));
		}
