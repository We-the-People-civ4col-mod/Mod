...
class CyXMLTextString
{
public:
	CyXMLTextString();
	CyXMLTextString(tinyxml2::XMLElement* m_pElement);

	std::wstring getText(bool bMale, bool bPlural) const;

	std::wstring getTextFromIndex(int iIndex) const;
	std::wstring getNameFromIndex(int iIndex) const;
...
{
public:
	TextFileStorage(const TCHAR* szPath, bool bVanilla);
	~TextFileStorage();

	int getNumTags() const {return m_TypeStrings.size();}
	const TCHAR* getTag(int i) const;

	tinyxml2::XMLElement* getElement(const TCHAR* szTag) const;
	tinyxml2::XMLElement* createElement(const TCHAR* szTag);

	void deleteElement(tinyxml2::XMLElement* pElement);
...
private:
	...
	ElementCacheType m_TypeCache;
	std::vector<CvString> m_TypeStrings;
};
...
public:
	...
	const wchar* getInfoWide(const TCHAR* szTag, const TCHAR* szSetting) const;
	...
	// TXT_KEY read
	tinyxml2::XMLElement* getText(const TCHAR* szTXT_KEY) const;

	// get the entire text xml file
	TextFileStorage* getTextXMLFile() const;

	// append languages to TEXT
	void addLanguages(tinyxml2::XMLElement *TEXT);
	...
	// generates a text xml file for each file (if it has strings in it)
...
public:
	...
	const CyXMLTextString* getTextString() const;
	...
	// write the values in a TXT_KEY
	void setTxtKey(const TCHAR* szMaleSingular, const TCHAR* szMalePlural, const TCHAR* szFemaleSingular, const TCHAR* szFemalePlural);
